...
let s:OPERATORFUNC = 'masquerade#impl#operatorfunc'
function! masquerade#impl#operatorfunc(motionwise) abort "{{{
...
function! masquerade#impl#edit(mode, cmd, options) abort "{{{
	if s:ms.isempty()
		let fallback = get(a:options, 'fallback', a:cmd)
		return fallback
	endif
	let l:Constructor = get(a:options, 'Constructor', function('s:MasqueradeEditor'))
	let msqrd = l:Constructor(a:mode, a:cmd)
	call msqrd.initialize(a:options)
	return msqrd.start()
endfunction "}}}
...
" y
function! masquerade#impl#y(mode, cmd, options) abort "{{{
	let a:options.Constructor = function('s:MasqueradeY')
	return masquerade#impl#edit(a:mode, a:cmd, a:options)
endfunction "}}}
...
" d
function! masquerade#impl#d(mode, cmd, options) abort "{{{
	let a:options.Constructor = function('s:MasqueradeD')
	return masquerade#impl#edit(a:mode, a:cmd, a:options)
endfunction "}}}
" MasqueradeD class{{{
let s:MasqueradeD = {
...
" !
function! masquerade#impl#exclamation(mode, cmd, options) abort "{{{
	if s:ms.isempty()
		let fallback = get(a:options, 'fallback', a:cmd)
		return fallback
	endif
...
" p
function! masquerade#impl#p(mode, cmd, options) abort "{{{
	let a:options.Constructor = function('s:MasqueradeP')
	return masquerade#impl#edit(a:mode, a:cmd, a:options)
endfunction "}}}
" MasqueradeP class{{{
let s:MasqueradeP = {
...
" i, a, c
function! masquerade#impl#i(mode, cmd, options) abort "{{{
	let i = s:MasqueradeI(a:mode, a:cmd)
	call i.initialize(a:options)
	call i.start()
endfunction "}}}
function! masquerade#impl#a(mode, cmd, options) abort "{{{
	let a = s:MasqueradeA(a:mode, a:cmd)
	call a.initialize(a:options)
	call a.start()
endfunction "}}}
function! masquerade#impl#c(mode, cmd, options) abort "{{{
	let c = s:MasqueradeC(a:mode, a:cmd)
	call c.initialize(a:options)
	call c.start()
endfunction "}}}
...
function! s:MasqueradeI.fallbackkeys() abort "{{{
	let keyseq = self._buildkeyseq('count', 'fallback')
	let flag = self.noremap ? 'in' : 'im'
	return [keyseq, flag]
endfunction "}}}
...
function! s:MasqueradeC.fallbackkeys() abort "{{{
	if self.mode is# 'x'
		let keyseq = self._buildkeyseq('gv', 'register', 'fallback')
	else
